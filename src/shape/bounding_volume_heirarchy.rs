...
impl BVHPrimitiveInfo {
    pub fn init(primitive_num: usize, bounds: Bounds3f) -> Self {
        let centroid: Point3 = na::center(&bounds.p_min, &bounds.p_max);
        Self {
            primitive_num,
            bounds,
            centroid
    ...
    pub fn init_interior(&mut self, axis: usize, left: Option<Box<BVHBuildNode>>, right: Option<Box<BVHBuildNode>>) {
        ...
        if let Some(left_bound) = &left {
            if let Some(right_bound) = &right {
                self.bounds = Bounds3f::union(&left_bound.bounds, &right_bound.bounds); // if both are not none
            } else {
...
impl BVHAccel {
    ...
    pub fn build(&mut self) {
        ...
        for i in 0..num_primitives {
            let primitive_info = BVHPrimitiveInfo::init(i, self.primitives[i].world_bound());
    ...
    fn recursively_build(&mut self, primtive_infos: &mut Vec<BVHPrimitiveInfo>, start: usize, end: usize, total_nodes: &mut usize, ordered_primitives: &mut Vec<Arc<dyn Primitive>>) -> BVHBuildNode {
        ...
        for i in start..end {
            bounds = Bounds3f::union(&bounds, &primtive_infos[i].bounds);
        }
        ...
        for i in start..end {
            centroid_bounds = Bounds3f::union_pt(&centroid_bounds, &primtive_infos[i].centroid);
        }
        ...
        SplitMethod::SAH => {
            if n_primitives <= 4 {
                ...
                for _ in 0..N_BUCKETS {
                    buckets.push(BucketInfo { count: 0usize, bounds: Bounds3f::new() });
                }
                ...
                for i in start..end {
                    ...
                    buckets[b].count += 1;
                    buckets[b].bounds = Bounds3f::union(&buckets[b].bounds, &primtive_infos[i].bounds);   
                }
    ...
    fn flatten_tree(&mut self, node: &BVHBuildNode, offset: &mut usize) -> usize {
        let cur_idx  = *offset;
        self.nodes[cur_idx].bounds = node.bounds;
        (*offset) += 1;
...
impl Primitive for BVHAccel {
    ...
    fn intersect(&self, ray: &mut Ray, isect: &mut SurfaceInteraction) -> bool {
        ...
        loop {
            ...
            if node.bounds.intersect_p_with_inv(ray, &inv_dir, dir_is_neg) {
                if node.n_primitives > 0 {
